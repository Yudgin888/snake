    ...
    public void focus(Point p) {
        if(p != null) {
            for(int i = 0; i < itMenu.size(); i++) {
                if((itMenu.get(i).getX() <= p.getX()-8) && ((itMenu.get(i).getX() + itMenu.get(i).getSizeX()) >= p.getX()-8)
                        && (itMenu.get(i).getY() <= p.getY()-25) && ((itMenu.get(i).getY() + itMenu.get(i).getSizeY()) >= p.getY()-25)){
                    itMenu.get(i).focus = true;
                } else {
                    itMenu.get(i).focus = false;
                }
            }
        }
    }
